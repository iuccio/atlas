package ch.sbb.atlas.servicepointdirectory.module.trafficpoint.search;

import ch.sbb.atlas.searching.SpecificationBuilder;
import ch.sbb.atlas.searching.specification.ValidOrEditionTimerangeSpecification;
import ch.sbb.atlas.servicepoint.Country;
import ch.sbb.atlas.servicepointdirectory.module.servicepoint.search.ServicePointNumberSboidSpecification;
import ch.sbb.atlas.servicepointdirectory.module.trafficpoint.entity.TrafficPointElementVersion;
import ch.sbb.atlas.servicepointdirectory.module.trafficpoint.entity.TrafficPointElementVersion_;
  ...
  public Specification<TrafficPointElementVersion> getSpecification() {
    return specificationBuilder().searchCriteriaSpecification(searchCriterias)
        ...
        .and(new ServicePointNumberSboidSpecification<>(
            ...
            trafficPointElementRequestParams.getServicePointNumbersShort().stream()
                .flatMap(str -> Arrays.stream(str.split(",")).map(String::trim)).map(Integer::valueOf).toList(),
            trafficPointElementRequestParams.getUicCountryCodes().stream()
                .flatMap(countryCode -> Arrays.stream(countryCode.split(",")).map(String::trim))
