...
class ServicePointStatusRevokedChangeNotAllowedExceptionTest {
  ...
  void shouldDisplayErrorMessage() {
    // given
    ServicePointStatusRevokedChangeNotAllowedException exception = new ServicePointStatusRevokedChangeNotAllowedException(
        new ServicePointNumber(12345678), Status.REVOKED);
    // when & then
    ...
    assertThat(errorResponse.getError()).isEqualTo(
        "Trying to update status for ServicePointNumber 1234567 and current status: REVOKED");
