...
public class CantonBasedUserAdministrationService extends BaseUserAdministrationService {
  ...
  public boolean isAtLeastExplicitReader(ApplicationType applicationType, CantonAssociated businessObject) {
    return isAtLeast(ApplicationRole.EXPLICIT_READER, applicationType, businessObject.getSwissCanton());
  }
  ...
  public boolean isAtLeastSupervisor(ApplicationType applicationType, CantonAssociated businessObject) {
    return isAtLeast(ApplicationRole.SUPERVISOR, applicationType, businessObject.getSwissCanton());
  }

  private boolean isAtLeast(ApplicationRole applicationRole, ApplicationType applicationType, SwissCanton swissCanton) {
    log.info("Checking if user {} may create/edit object for canton {}", getCurrentUserSbbUid(), swissCanton);
    if (isAdmin()) {
      return true;
    }
  ...
  private ApplicationRole getCantonBasedUserPermissions(ApplicationType applicationType, SwissCanton swissCanton) {
    ...
    if (userPermissionsForApplication.getRole() == ApplicationRole.WRITER) {
      if (userPermissionsForApplication.getSwissCantons().contains(swissCanton)) {
        return ApplicationRole.WRITER;
      } else {
        return ApplicationRole.READER;
      }
    }
