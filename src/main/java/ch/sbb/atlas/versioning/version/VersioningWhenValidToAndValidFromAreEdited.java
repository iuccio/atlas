...
import static ch.sbb.atlas.versioning.version.VersioningHelper.areBothValidToAndValidFromChanged;
import static ch.sbb.atlas.versioning.version.VersioningHelper.areValidToAndPropertiesEdited;
import static ch.sbb.atlas.versioning.version.VersioningHelper.findObjectToVersioningInValidFromValidToRange;
...
import static ch.sbb.atlas.versioning.version.VersioningHelper.hasNextVersion;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isBetweenMultipleVersionsAndOverTheBorders;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isCurrentVersionBetweenEditedValidFromAndEditedValidTo;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedValidFromAfterCurrentValidFromAndBeforeCurrentValidTo;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedValidFromExactOnTheLeftBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedValidFromOverTheLeftBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedValidToAfterTheRightBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedValidToExactOnTheRightBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedValidToOverTheRightBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedVersionExactMatchingMultipleVersions;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isEditedVersionInTheMiddleOfCurrentVersion;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isOnTheLeftBorderAndEditedValidFromIsBeforeTheLeftBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isOnTheRightBorderAndEditedEntityIsOnOrOverTheBorder;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isOnlyValidToChanged;
import static ch.sbb.atlas.versioning.version.VersioningHelper.isOnlyValidToEditedWithNoEditedProperties;
...
public class VersioningWhenValidToAndValidFromAreEdited extends Versioning {
  ...
  public List<VersionedObject> applyVersioning(Versionable editedVersion,
    ...
    if (objectToVersioningFound.isEmpty()) {
      List<VersionedObject> versionedObjectsOnNoObjectFound = applyVersioningWhenNoEntityFound(
          editedValidFrom, editedValidTo, editedEntity, objectsToVersioning);
      ...
      List<VersionedObject> versionedObjectsOnOnlyOneObjectFound = applyVersioningOnSingleFoundEntity(
          editedValidFrom, editedValidTo, editedVersion, currentVersion, editedEntity,
  ...
  private List<VersionedObject> applyVersioningOnSingleFoundEntity(LocalDate editedValidFrom,
      LocalDate editedValidTo, Versionable editedVersion,
    ...
    if (isEditedVersionInTheMiddleOfCurrentVersion(editedValidFrom, editedValidTo, toVersioning)) {
      List<VersionedObject> versionedObjectsInTheMiddleOfAnExistingEntity = applyVersioningToAnObjectInTheMiddleOfAnExistingEntity(
          editedValidFrom, editedValidTo, currentVersion, editedEntity,
          toVersioning);
      ...
      List<VersionedObject> versionedObjectsOnTheBorder = applyVersioningOnTheBorderOnSingleFoundEntity(
          editedValidFrom, editedValidTo, editedVersion,
          editedEntity, toVersioning);
  ...
  private List<VersionedObject> applyVersioningWhenNoEntityFound(LocalDate editedValidFrom,
  ...
  private List<VersionedObject> applyVersioningOverMultipleFoundEntities(LocalDate editedValidFrom,
      LocalDate editedValidTo, Versionable editedVersion, Versionable currentVersion,
      Entity editedEntity,
    ...
    if (isThereGapBetweenVersions(toVersioningList)) {
      return applyVersioningWhenThereIsGapBetweenVersionsFound(editedValidFrom, editedValidTo,
          editedVersion, currentVersion, editedEntity,
          toVersioningList);
    }
    if (isBetweenMultipleVersionsAndOverTheBorders(editedValidFrom, editedValidTo,
      ...
      applyVersioningOnLeftBorderWhenValidFromIsAfterCurrentValidFrom(editedValidFrom, editedEntity,
          toVersioningList, versionedObjects);

      applyVersioningOnTheLeftBorderWhenValidToIsBeforeCurrentValidTo(editedValidTo, editedEntity,
          toVersioningList, versionedObjects);
    ...
    if(isEditedValidFromOverTheLeftBorder(editedValidFrom,toVersioningList) || isEditedValidToOverTheRightBorder(editedValidTo,toVersioningList)){
      if(isEditedValidFromOverTheLeftBorder(editedValidFrom,toVersioningList)){
        ...
        applyVersioningOnLeftBorderWhenValidFromIsAfterCurrentValidFrom(editedValidFrom,
            editedEntity, toVersioningList, versionedObjects);
      }else {
        throw new VersioningException(
            "Something went wrong. I'm not able to apply versioning on this scenario.");
      }
    ...
    if (isOnlyValidToChanged(editedVersion, currentVersion)
      ...
      for (int i = 0; i < toVersioningList.size(); i++) {
        ...
        if (hasNextVersion(toVersioningList, i)) {
          ...
          } else if (DateHelper.areDatesSequential(current.getVersionable().getValidTo(),
              next.getVersionable().getValidFrom())) {
