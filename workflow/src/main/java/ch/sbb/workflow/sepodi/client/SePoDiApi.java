...
public interface SePoDiApi {
  ...
  // Termination endpoints in internal? mit admin securen?
  ...
  @PutMapping(value = BASEPATH + "/termination/stop/{sloid}/{id}")
  ReadServicePointVersionModel terminateStopPoint(@PathVariable("sloid") String sloid,
      @PathVariable("id") Long id);

  @PutMapping(value = BASEPATH + "/termination/stop/{sloid}/{id}")
  ReadServicePointVersionModel changeToTariffStop(@PathVariable("sloid") String sloid,
      @PathVariable("id") Long id);
