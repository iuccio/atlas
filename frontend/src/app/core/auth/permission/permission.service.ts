...
export class PermissionService {
  ...
  getApplicationUserPermission(applicationType: ApplicationType) {
    return PermissionService.getApplicationPermission(
      this.permissions,
      applicationType
    );
  }
  ...
  isAtLeastSupervisor(applicationType: ApplicationType) {
    const applicationUserPermission =
      this.getApplicationUserPermission(applicationType);
    return (
      this.isAdmin ||
      applicationUserPermission.role === ApplicationRole.Supervisor
    );
  }
  ...
  mayAccessBulkImport() {
    return (
      this.isAdmin ||
      BulkImportPermission.hasAnyBulkImportPermission(this.permissions)
    );
  }
  ...
  static hasPermissionToWriteOnCanton(
    ...
    const applicationUserPermission =
      PermissionService.getApplicationPermission(permissions, applicationType);
    if (
      isAdmin ||
      applicationUserPermission.role === ApplicationRole.Supervisor
    ) {
    ...
    if (applicationUserPermission.role === ApplicationRole.Writer) {
      const allowedSwissCantons =
        applicationUserPermission.permissionRestrictions.map(
          (restriction) => restriction.valueAsString
        );
  ...
  static hasPermissionsToWriteWithPermissions(
    ...
    if (
      PermissionService.getRolesAllowedToUpdate(applicationType).includes(
        applicationPermission.role
      )
    ) {
      return true;
    ...
    if (sboid && ApplicationRole.Writer === applicationPermission.role) {
      return PermissionService.getSboidRestrictions(
