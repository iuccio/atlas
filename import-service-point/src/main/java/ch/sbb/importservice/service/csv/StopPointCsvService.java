...
public class StopPointCsvService extends CsvService<StopPointCsvModel> {
  ...
  protected StopPointCsvService(FileHelperService fileHelperService, JobHelperService jobHelperService) {
  ...
  protected String getImportCsvJobName() {
    return JobDescriptionConstants.IMPORT_STOP_POINT_CSV_JOB_NAME;
  }

  @Override
  protected Class<StopPointCsvModel> getType() {
    return StopPointCsvModel.class;
  }

  public List<StopPointCsvModelContainer> mapToStopPointCsvModelContainers(
      List<StopPointCsvModel> stopPointCsvModels) {

    List<StopPointCsvModel> activeStopPoints = filterStopPoints(stopPointCsvModels);

    Map<Integer, List<StopPointCsvModel>> stopPointsGroupedByDidokCode = activeStopPoints.stream()
        .collect(Collectors.groupingBy(StopPointCsvModel::getDidokCode));
    List<StopPointCsvModelContainer> stopPointCsvModelContainers = new ArrayList<>();
    stopPointsGroupedByDidokCode.forEach((key, value) -> {
      StopPointCsvModelContainer servicePointCsvModelContainer = StopPointCsvModelContainer.builder()
          .didokCode(key)
          .stopPointCsvModels(value)
          .build();
      value.sort(Comparator.comparing(BasePrmCsvModel::getValidFrom));
      stopPointCsvModelContainers.add(servicePointCsvModelContainer);
    });
