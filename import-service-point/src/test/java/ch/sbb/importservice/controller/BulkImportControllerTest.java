...
class BulkImportControllerTest extends BaseControllerApiTest {
  ...
  void shouldAcceptGenericBulkImportWithFile() throws Exception {
    Path file = Files.createTempFile("tmp", ".csv");
    mvc.perform(multipart("/v1/import/bulk/SEPODI/SERVICE_POINT/UPDATE")
            .file(new MockMultipartFile("file", "import.csv", "application/csv", Files.readAllBytes(file))))
    ...
    assertThat(bulkImport.getImportFileUrl()).isEqualTo(todaysDirectory + "/import.csv");

    verify(amazonService).putFile(eq(AmazonBucket.BULK_IMPORT), any(File.class), eq(todaysDirectory));
  }
}