package ch.sbb.prm.directory.module.contactpoint.service;
...
class ContactPointServiceTest extends BasePrmServiceTest {
  ...
  void shouldCreateContactPointWhenNoReferencePointExists() {
    ...
    assertThat(contactPointVersions.getFirst().getParentServicePointSloid()).isEqualTo(
  ...
  void shouldCreateInformationDeskWhenReferencePointExists() {
    ...
    assertThat(relationVersions.getFirst().getReferencePointElementType()).isEqualTo(ReferencePointElementType.CONTACT_POINT);
    assertThat(relationVersions.getFirst().getParentServicePointSloid()).isEqualTo(PARENT_SERVICE_POINT_SLOID);
    assertThat(relationVersions.getFirst().getReferencePointElementType()).isEqualTo(ReferencePointElementType.CONTACT_POINT);
    verify(prmLocationService, times(1)).allocateSloid(any(), eq(SloidType.CONTACT_POINT));
  }
