package ch.sbb.prm.directory.module.parkinglot.service;
...
import ch.sbb.prm.directory.module.parkinglot.search.ParkingLotSearchRestrictions;
...
class ParkingLotServiceTest extends BasePrmServiceTest {
  ...
  void shouldCreateParkingLotWhenNoReferencePointExists() {
    ...
    assertThat(parkingLotVersions.getFirst().getParentServicePointSloid()).isEqualTo(parkingLot.getParentServicePointSloid());
  ...
  void shouldCreateParkingLotWhenReferencePointExists() {
    ...
    assertThat(relationVersions.getFirst().getReferencePointElementType()).isEqualTo(PARKING_LOT);
    verify(prmLocationService, times(1)).allocateSloid(any(), eq(SloidType.PARKING_LOT));
  }
