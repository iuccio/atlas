...
import ch.sbb.prm.directory.module.stoppoint.exception.StopPointVariantChangingNotAllowedException;
import ch.sbb.prm.directory.module.stoppoint.service.StopPointValidationService;
...
class StopPointValidationServiceTest {
  ...
  void shouldNotValidateWhenReducedPRMContainsAllFields() {
    ...
    assertThat(errorResponse.getMessage()).isEqualTo(
        "StopPointVersion with sloid [ch:1:sloid:12345] cannot be save: Attempting to save a Reduced object with wrong "
  ...
  void shouldNotValidateWhenCompletePRMDoesNotContainsAllMandatoryFields() {
    ...
    assertThat(errorResponse.getStatus()).isEqualTo(400);
    assertThat(errorResponse.getMessage()).isEqualTo(
        "StopPointVersion with sloid [ch:1:sloid:12345] cannot be save: Attempting to save a Complete object with wrong "
            + "properties population!");
